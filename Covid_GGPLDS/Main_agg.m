...
elseif strcmp(dataset_name,'FHZ')
 ...
 elseif strcmp(dataset_name,'Covid19_newcases')
    ...
    data1 =TT1(1:end,[1,55:size(TT,2)]);
    data2 =TT1{2:end,55:end};
    data3 =TT2(1:end,[1,55:size(TT,2)]);
    data4 =TT2{2:end,55:end};
    data7=[];
    data7=[data5; cellfun(@str2num, data2(:,1:end))];
    data6=[];
    ...
    data1{end,2:end}=num2cell(sum(data7,1));
    data3{2:end,2:end}=num2cell(data6);
    data3(end+1,1)=cellstr('US');
    data3{end,2:end}=num2cell(sum(data6,1));
...
for iteration = 1:Max_MCMC
    ...
    if (iteration >Burnin && rem(iteration,10)==0)
        r_col(:,kk) =r;
        m_col(:,:,:,kk) = m_ikj;
        theta_col(:,:,kk) = theta;
        psi_col(:,:,kk)=psi;
...
if ~ strcmp(dataset_name,'Covid19_newcases')
    ...
    if (strcmp(task,'prediction_historic')||strcmp(task,'prediction_future'))
        ...
        US_total_obse_death = sum(data5(1:51,1:T_S),1);
        US_total_obse_cases = sum(data5(52:102,1:T_S),1);
        US_recon_avg_death = squeeze(sum(X_recon_avg(1:51,:),1));
        US_recon_avg_cases = squeeze(sum(X_recon_avg(52:end),1));
        US_total_pred_death = (sum(XP_temp(1:51,:,:,:),1));
        US_total_pred_cases = (sum(XP_temp(52:end,:,:,:),1));
        XP_temp =cat(1,XP_temp,US_total_pred_death);
        XP_temp =cat(1,XP_temp,US_total_pred_cases);
        US_total_pred_death = permute(squeeze(US_total_pred_death),[1 3 2]);
        US_total_pred_cases = permute(squeeze(US_total_pred_cases),[1 3 2]);
        US_avg_cases = squeeze(mean(squeeze(mean(US_total_pred_cases,2)),1));
        US_avg_death = squeeze(mean(squeeze(mean(US_total_pred_death,2)),1));
        state_means = [state_means;US_avg_death;US_avg_cases];
        US_avg = [US_total_obse,US_avg_death_US_avg_cases];
        MyX = cat(1,MyX,US_avg);
        if (strcmp(task,'prediction_historic'))
            TrX = cat(1, TrX, sum(data5(1:51,:),1));
            TrX = cat(1, TrX, sum(data5(52:end,:),1));
