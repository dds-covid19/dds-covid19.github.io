function [XP_temp,abser1,XP_temp2,abser2] = Prediction_Ksteps_count(X,X_w,T_P,X_dim_S,S0_col,S_col,Phi_prime_col,K,W_col,Z_col,D_col,X_W_avg,T,rr_col,task)
        ...
        %     SP_prev = SP_temp;
            ...
            if t == 1
                ...
                for j=1:pred_count
                    ...
                    XP_temp(:,j,t,i) =  nbinrnd(rr_col(:,i),1-sigmoid(D_col(:,:,i)*SP_temp(:,j)));
                end
    ...
    %                 XP_temp1 = mvnrnd((D_col(:,:,i)*SP_temp')',inv(Phi_col(:,:,i)))';
                %XP1(:,t,i) = mean(XP_temp(:,:,t,i),2);
                %abser1(:,t,i) = abs(XP1(:,t,i)- X_W_avg(:,t));

            end
            XP_avg_temp = squeeze(mean(XP_temp,2));
            if strcmp(task,'predeiction_historic')
                abser1(:,t,i) = mean(abs(XP_avg_temp(:,t,i)- X_W_avg(:,t))./(X_W_avg(:,t)+1),2);
            else
                abser1=[];
            end
            %abser1(:,t,i) = mean(abs(XP_temp(:,:,t,i)- X_W_avg(:,t))./(X_W_avg(:,t)+1),2);
            SP_prev = SP_temp;
        end
        %X_pred(:,:,i) = 
    end
  ...
  pred_count = 100;
    ...
    for i = 1:size(S_col,3)
        ...
        %     SP_prev = SP_temp;
            ...
            if t == 1
                ...
                for j=1:pred_count
                    %z = mvnrnd(zeros(X_dim_S,1),eye(X_dim_S));
                    XP_temp2(:,j,t,i)= rr_col(:,i).* exp(D_col(:,:,i)*SP_temp(:,j));
                    %XP_temp2(:,j,t,i) =  nbinrnd(rr_col(:,i),1-sigmoid(D_col(:,:,i)*SP_temp(:,j)));
                end
                %XP_temp1 = mvnrnd((D_col(:,:,i)*SP_temp')',inv(Phi_col(:,:,i)))';
                %XP1(:,t,i) = mean(XP_temp,2);
                %abser1(:,t,i) = abs(XP1(:,t,i)- X_W_avg(:,t));
    ...
    %                 XP_temp1 = mvnrnd((D_col(:,:,i)*SP_temp')',inv(Phi_col(:,:,i)))';
            ...
            if strcmp(task,'predeiction_historic')
                abser2(:,t,i) = mean(abs(XP_temp2(:,:,t,i)- X_W_avg(:,t))./(X_W_avg(:,t)+1),2);
            else
                abser2=[];
            end
