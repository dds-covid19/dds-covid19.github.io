function [D,S,S0,l4,omega,rr] = sample_SD_Omega_rr(y,X,W,Z,Phi_prime,S0,S,D,omega,rr,l4,Model_hyper_params,Model_properties)
    %y= data;
   ...
   Truncation = 5;
    ...
    %%%%% samplr rr
    l4_ps = sum(l4,2);
    p4 = sum(log(1+exp(Psi)),2);
    par2 =1/(beta_r+p4);
    for p = 1:X_dim_S 
       par1 = l4_ps(p) + alpha_r;
       rr(p) = gamrnd(par1,par2(p));
    end
    
   %%%%%%%polyagamma sampling
   for p=1:X_dim_S
       for t=1:T
           omega(p,t)= PolyaGamRnd_Gam(y(p,t)+rr(p),D(p,:)*S(:,t),Truncation);
       end
   end
   %%%%%%%%%%% define vector v
   V = (y-rr)/2.0;
