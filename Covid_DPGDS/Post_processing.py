...
save_file = './results/' + foldername + '_prediction/' + 'layer' + str(1) + '/K1_' + str(20) + '/'

if event == 'death':
    death_mean = pd.read_csv(save_file+event+'_mean_.csv')
    death_mean = death_mean.rename(columns=death_mean.loc[0,:])
    death_mean=(death_mean.drop(index=0))#.drop('03/15/2020',axis=1)
    death_lower = pd.read_csv(save_file+event+'_lowerBound_.csv')
    death_lower = death_lower.rename(columns=death_lower.loc[0,:])
    death_lower = (death_lower.drop(index=0))#.drop('01/22/0020',axis=1)

    death_higher = pd.read_csv(save_file+event+'_upperBound_.csv')
    ...
    death_mean.to_csv (save_file+event+'_mean_.csv', index = False, header=True)
    death_lower.to_csv (save_file+event+'_lowerBound_.csv', index = False, header=True)
    death_higher.to_csv (save_file+event+'_upperBound_.csv', index = False, header=True)
    ...
    results_death.to_csv (save_file+foldername+'__death_.csv', index = True, header=True)
    ...
    name='daily_cases'
    cases_mean = pd.read_csv(save_file+name+'_mean_.csv')

    cases_mean = cases_mean.rename(columns=cases_mean.loc[0,:])
    cases_mean=(cases_mean.drop(index=0))#.drop('01/22/0020',axis=1)
    cases_lower = pd.read_csv(save_file+name+'_lowerBound_.csv')
    cases_lower = cases_lower.rename(columns=cases_lower.loc[0,:])
    cases_lower = (cases_lower.drop(index=0))#.drop('01/22/0020',axis=1)
    cases_higher = pd.read_csv(save_file+name+'_upperBound_.csv')
    cases_higher = cases_higher.rename(columns=cases_higher.loc[0,:])
    ...
    cases_mean.to_csv (save_file+name+'_mean_.csv', index = False, header=True)
    cases_lower.to_csv (save_file+name+'_lowerBound_.csv', index = False, header=True)
    cases_higher.to_csv (save_file+name+'_upperBound_.csv', index = False, header=True)
    ...
    results_cases.to_csv (save_file+foldername+'__daily_cases_.csv', index = True, header=True)
