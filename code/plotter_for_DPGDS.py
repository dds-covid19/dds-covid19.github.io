...
if event == 'death':
    death_ = pd.read_csv( '../Covid_DPGDS/results/' + foldername + '_prediction/' +'layer1/K1_20/'+ foldername + '__' + sfilename + '_.csv').drop(
        'Unnamed: 0', axis=1)
    ...
    cases_ = pd.read_csv(
        '../Covid_DPGDS/results/' + foldername + '_prediction/' +'layer1/K1_20/'+ foldername + '__daily_cases_.csv').drop('Unnamed: 0',
                                                                                                           axis=1)
    ...
    for i in range(52):
        ...
        y_prediction = temp['number_of_daily_cases'].iloc[-7:]

        y_lower = temp['number_of_daily_cases_lower'].iloc[-7:]

        y_higher = temp['number_of_daily_cases_higher'].iloc[-7:]

        x_fit = temp['date'].iloc[52:-7]

        y_fit = temp['number_of_daily_cases'].iloc[52:-7]
...
fig.write_html('../images/DPGDS/fig_' + foldername + '_' + sfilename + '.html')